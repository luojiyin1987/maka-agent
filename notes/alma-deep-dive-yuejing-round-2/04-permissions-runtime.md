# 04 — Alma permission runtime (tool approval machinery)

> WAWQAQ called out Maka's three-mode 只读 / 确认 / 执行 design as
> "傻逼 / 多余" (msg `bdb272f7`). This note traces alma's actual
> implementation against `~/Downloads/alma-re/readable/main.js` so
> we have a source-grounded reference for what to converge to.

## Punchline

Alma's permission runtime is **binary, not three-mode**:
...
surface for it is the Shift+Tab toggle alma is known for.
...
Concrete simplifications that match alma:
...
2. **Add `allow_always` to the dialog's choice set.** Currently
   ...
   persists across turns matches alma's semantics and unblocks
...
How does alma decide WHICH tools auto-approve when
...
- Round 1: [`09-symbol-recovery.md`](../alma-deep-dive-yuejing-2026-05-31/09-symbol-recovery.md)
